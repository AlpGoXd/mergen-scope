  ...
  return (
    <div style={{ border: '1px solid var(--border)', borderRadius: '8px', overflow: 'hidden' }}>
      <button type="button" onClick={onToggle} style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 10px', border: 'none', background: 'var(--bg)', color: 'var(--text)', cursor: 'pointer' }}>
        <Sec first>{title}</Sec>
        <span style={{ fontSize: 'var(--font-label)', lineHeight: 'var(--lh-label)', color: 'var(--dim)' }}>{open ? '-' : '+'}</span>
      </button>
    ...
    <AnalysisFeatureCard title="Trace Ops" description="Create derived traces with offset, scale, smoothing, and trace math.">
      <div style={{ display: 'grid', gap: '10px' }}>
        <CollapsibleSection title="Offset" open={traceOpsOpenSections.offset} onToggle={() => toggleSection('offset')}>
          ...
          <label style={{ display: 'grid', gap: '4px', fontSize: 'var(--font-label)' }}><span style={{ color: 'var(--muted)', fontWeight: 400 }}>dB</span><input type="number" step="0.1" value={offsetValue} onChange={(event) => setOffsetValue(event.target.value)} style={INPUT_STYLE} /></label>
        ...
        <CollapsibleSection title="Scale" open={traceOpsOpenSections.scale} onToggle={() => toggleSection('scale')}>
          ...
          <label style={{ display: 'grid', gap: '4px', fontSize: 'var(--font-label)' }}><span style={{ color: 'var(--muted)', fontWeight: 400 }}>×</span><input type="number" step="0.1" value={scaleValue} onChange={(event) => setScaleValue(event.target.value)} style={INPUT_STYLE} /></label>
        ...
        <CollapsibleSection title="Smoothing" open={traceOpsOpenSections.smoothing} onToggle={() => toggleSection('smoothing')}>
          ...
          <label style={{ display: 'grid', gap: '4px', fontSize: 'var(--font-label)' }}>
            <span style={{ color: 'var(--muted)', fontWeight: 400 }}>Method</span>
          ...
          <label style={{ display: 'grid', gap: '4px', fontSize: 'var(--font-label)' }}><span style={{ color: 'var(--muted)', fontWeight: 400 }}>Window</span><input type="number" min="3" step="2" value={smoothWindow} onChange={(event) => setSmoothWindow(event.target.value)} style={INPUT_STYLE} /></label>
          {smoothMethod === 'savitzky-golay' && <label style={{ display: 'grid', gap: '4px', fontSize: 'var(--font-label)' }}><span style={{ color: 'var(--muted)', fontWeight: 400 }}>Poly order</span><input type="number" min="1" step="1" value={smoothPolyOrder} onChange={(event) => setSmoothPolyOrder(event.target.value)} style={INPUT_STYLE} /></label>}
        ...
        <CollapsibleSection title="Trace Math" open={traceOpsOpenSections.subtract} onToggle={() => toggleSection('subtract')}>
          <label style={{ display: 'grid', gap: '4px', fontSize: 'var(--font-label)' }}>
            <span style={{ color: 'var(--muted)', fontWeight: 400 }}>Operation</span>
          ...
          <label style={{ display: 'grid', gap: '4px', fontSize: 'var(--font-label)' }}>
            <span style={{ color: 'var(--muted)', fontWeight: 400 }}>Interpolation</span>
          ...
          {compatibility && !compatibility.compatible && <div style={{ fontSize: 'var(--font-label)', lineHeight: 'var(--lh-label)', color: '#ff6b6b' }}>{compatibility.warning ?? 'Trace units are not compatible.'}</div>}
          {compatibility && compatibility.compatible && compatibility.warning && <div style={{ fontSize: 'var(--font-label)', lineHeight: 'var(--lh-label)', color: 'var(--muted)' }}>{compatibility.warning}</div>}
          {error && <div style={{ fontSize: 'var(--font-label)', lineHeight: 'var(--lh-label)', color: '#ff6b6b' }}>{error}</div>}
