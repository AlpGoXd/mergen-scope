...
function useAppController(){
  ...
  var wizards=fileCtl.wizards, resolveWizard=fileCtl.resolveWizard, cancelWizard=fileCtl.cancelWizard, openWizardForFile=fileCtl.openWizardForFile;
  ...
  function onPaneDrop(paneId,ev){
    ...
    if(traceName) {
      var droppedTrace = getTraceByName(traceName);
      if(droppedTrace) {
        var dropDomain = droppedTrace.domain || "frequency";
        var existingTraces = allTr.filter(function(t){ return getTracePaneId(tracePaneMap, t.name) === paneId; });
        var hasConflict = false;
        for(var i=0; i<existingTraces.length; i++) {
           var eDomain = existingTraces[i].domain || "frequency";
           if(eDomain !== dropDomain) { hasConflict = true; break; }
        }
        if(!hasConflict){
          moveTraceToPane(traceName,paneId);
        } else {
          setError("Cannot mix Time-Domain and Frequency traces in the same plot pane!");
        }
      }
    }
    setDragTraceName(null);
  ...
  return {state:{isDrag:isDrag,setIsDrag:setIsDrag,showSidebar:showSidebar,showImportExportPanel:showImportExportPanel,error:error,wizards:wizards},derived:{hasData:hasData,rightPanelSections:rightPanelSections},refs:{fRef:fRef,iRef:iRef,wRef:wRef},actions:{importWorkspaceFile:importWorkspaceFile,resolveWizard:resolveWizard,cancelWizard:cancelWizard},chartHandlers:{onDrop:onDrop,loadFiles:loadFiles},view:{topBarProps:topBarProps,sidebarProps:sidebarProps,toolbarProps:toolbarProps,chartWorkspaceProps:chartWorkspaceProps,analysisStackProps:analysisStackProps,importExportPanelProps:importExportPanelProps,dataTableProps:dataTableProps,footerProps:footerProps,rightPanelSections:rightPanelSections}};
}
...
function AppRoot(){
  ...
  var view=app.view;
  var ImportWizardModal=window.ImportWizardComponents ? window.ImportWizardComponents.ImportWizardModal : null;
  return h("div",{style:{background:"var(--bg)",color:"var(--text)",height:"100vh",display:"flex",flexDirection:"column"},onDragOver:function(ev){ev.preventDefault();if(isFileDragEvent(ev))state.setIsDrag(true);},onDragLeave:function(ev){if(!ev.currentTarget.contains(ev.relatedTarget))state.setIsDrag(false);},onDrop:chartHandlers.onDrop},
    ImportWizardModal && state.wizards && state.wizards.length > 0 ? h(ImportWizardModal, { wizard: state.wizards[0], onConfirm: actions.resolveWizard, onCancel: actions.cancelWizard }) : null,
